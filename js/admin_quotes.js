...
async function loadQuotes() {
  try {
    ...
    // ✅ LOGGER L'ERREUR
    if (window.ActivityLogger) {
      await window.ActivityLogger.logError('load_quotes', error.message);
    }
...
document.getElementById('quote-form').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    // ✅ LOGGER LA CRÉATION DU DEVIS
    if (window.ActivityLogger && data && data[0]) {
      const clientName = clientOption.dataset.name || clientOption.dataset.email;
      await window.ActivityLogger.logCreateQuote(
        data[0].id,
        clientName,
        totalTtc
      );
    ...
    // ✅ LOGGER L'ERREUR
    if (window.ActivityLogger) {
      await window.ActivityLogger.logError('create_quote', error.message);
    }
...
async function sendQuote(quoteId) {
  ...
  try {
    ...
    // ✅ LOGGER L'ENVOI DU DEVIS
    if (window.ActivityLogger && quote) {
      const clientName = quote.profiles?.full_name || quote.client_name || quote.client_email;
      await window.ActivityLogger.logSendQuote(quoteId, clientName);
    }
    ...
    // ✅ LOGGER L'ERREUR
    if (window.ActivityLogger) {
      await window.ActivityLogger.logError('send_quote', error.message);
    }
...
function viewQuote(quoteId) {
  ...
  if (window.ActivityLogger) {
    window.ActivityLogger.log('view_quote', 'quote', quoteId, {
      description: 'Consultation détail devis'
    });
  }
