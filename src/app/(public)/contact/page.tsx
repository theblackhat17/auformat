...
import { getSettings, getAvisStats } from '@/lib/content';
...
export default async function ContactPage() {
  const [settings, avisStats] = await Promise.all([getSettings(), getAvisStats()]);
    ...
    <>
      ...
      <section className="bg-noir text-white py-24">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <p className="text-white/80 text-lg max-w-2xl">Devis gratuit et sans engagement. Réponse sous 24h.</p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="grid lg:grid-cols-3 gap-12">
            ...
            <div className="space-y-6">
              {[
                { icon: '📍', title: 'Atelier Cysoing', content: address, sub: 'Près de Lille (Nord) · Sur rendez-vous' },
                { icon: '📍', title: 'Atelier Côte d\'Opale', content: '1056 Rue de Montreuil, 62170 La Calotterie', sub: 'Près du Touquet (Pas-de-Calais) · Sur rendez-vous' },
                { icon: '📞', title: 'Téléphone', content: phone, sub: hoursWeekdays },
                { icon: '✉️', title: 'Email', content: email, sub: 'Réponse sous 24h' },
              ].map((info) => (
                <div key={info.title} className="flex gap-4 p-4 bg-beige/50 rounded-xl">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-noir/60 mt-0.5">{info.sub}</p>
                  </div>
                </div>
              ))}
              ...
              <div className="p-6 bg-vert-foret/5 rounded-xl border border-vert-foret/10">
                <h3 className="text-sm font-semibold text-noir mb-3">Besoin d&apos;une réponse rapide ?</h3>
