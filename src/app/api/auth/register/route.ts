...
import { sendWelcomeEmail, notifyNewRegistration } from '@/lib/mailer';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Send welcome email to client + notify admin
    sendWelcomeEmail(profile.email, profile.fullName || 'Client').catch(() => {});
    notifyNewRegistration(profile.fullName || 'Client', profile.email).catch(() => {});
