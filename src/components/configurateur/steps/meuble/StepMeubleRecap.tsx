...
import { WOOD_MATERIALS, HANDLE_TYPES, HINGES_CATALOG, DRAWER_SLIDES_CATALOG, EDGE_BANDING_CATALOG, FINISHES_CATALOG } from '@/lib/constants';
...
export function StepMeubleRecap({ config, price }: Props) {
  ...
  const totalModules = config.cabinets.reduce((s, c) => s + c.modules.length, 0);
    ...
    <div className="p-6">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 mb-4">
        ...
        <div className="grid grid-cols-2 gap-y-1 text-sm">
          ...
          <span className="text-gray-500">Caissons</span>
          <span className="text-noir">{config.cabinets.length}</span>
          <span className="text-gray-500">Modules</span>
          <span className="text-noir">{totalModules}</span>
      ...
      {/* Hardware summary */}
      <div className="bg-white border border-gray-200 rounded-xl p-4 mb-4">
        <h3 className="font-semibold text-sm text-noir mb-2">Quincaillerie</h3>
        <div className="grid grid-cols-2 gap-y-1 text-sm">
